...
- [x] ✅ **G1.3 — DOWNGRADED & resolved (iter 11).** Re-tested at **true 390px** (device emulation): the R2 "critical" symptoms (time-overprints-title, dual scrollbars, half-empty modal) were **500px non-mobile artifacts and did NOT reproduce** — the mobile body is clean (time renders at card bottom). The one real issue was the timeline **minimap overlapping the Stream View header**; fixed with `max-md:hidden` on the minimap while the stream modal is open. Mobile landing: no horizontal overflow. `[was CRIT → MINOR · done]`
- [x] ✅ **G1.4** (iter 12) "Invalid Date" rendering fixed — `formatDateSafe()` in `lib/time.ts`, applied in HomePage + UserProfilePage; invalid/missing dates render "—". *(Root data cause why some `updatedAt` are invalid remains G7.9.)* `[MED · S · seo]`
...
- **Iter 18** (visual polish — skeletons): Confirmed the card loading skeletons (`SkeletonCard.css`) are **already Calm Modern** — token-driven (`--card-bg`/`--page-bg-elevated`/`--card-border`), correct in light + dark (the iter-1 token foundation flowed through). One fix: the timeline minimap's Suspense fallback used hardcoded `bg-gray-200` (clashed in dark) → now `var(--page-bg-elevated)`. Smoke clean. *Loop now at diminishing returns — proposing wind-down; remaining big items G3.1/G2.1 need owner input.*
- **Iter 17** (content/trust — G5.5): Footer cleanup — removed the internal "Built with PowerSpawn — AI-powered multi-agent orchestration" dev reference from the public footer; added a "Create a timeline" link so the Product column isn't a lone link; relabeled the raw personal Gmail to "Email us" (mailto preserved). Smoke clean. *Next: loading/skeleton polish, featured ordering G5.6, or micro-interactions; G3.1/G2.1 await owner input.*
- **Iter 16** (visual — G4.7): Added a subtle **event-count density bar** to the browse/discovery timeline cards (`EventCount` component in HomePage; log-scaled indigo fill vs a ~300-event reference, clamped 8–100%; replaced all 4 `{eventCount} events` spans). Cards now reflect scale at a glance — French Revolution (250/295) reads near-full, RFK (10) short, empty timelines minimal — addressing Round 1's "visually interchangeable" finding. Verified across varied counts on /browse. Smoke clean. *Next: continued safe polish (loading/skeletons, footer trust cleanup, featured ordering G5.6); big items G3.1/G2.1 await owner input.*
- **Iter 15** (consolidation + G4.7): **Full production `npm run build` PASSED** (`✓ built in 17.49s`, 2140 modules, PWA SW generated) — all branch commits bundle cleanly (pre-existing chunk-size/browserslist warnings only). Small win: hid the redundant **@username pill on the owner's own profile** cards (username already in the page header) — declutter; kept the badge for any non-owner case. Verified the profile is clean Calm Modern (and confirmed live: "—" date fallback, indigo avatar, indigo CTAs). Smoke clean. *Next: Stream View as first-class deep-linkable view (G3.1) or continued polish.*
- **Iter 14** (visual — G4.6 dark parity): Verified the **dark** theme is coherent Calm Modern (charcoal bg, indigo CTA + icon badges, feature-card accents) — the token work already carried into dark. One fix: raised the dark **card surface** `--card-bg` `#1c1b19`→`#232220` + border `#353330` so cards separate clearly from the bg and elevated bands (persona/feature/capability/browse cards). Smoke clean. *Next: browse/profile card hooks (G4.7), Stream View as first-class view (G3.1), or `npm run build` verification.*
- **Iter 13** (functional — auth-race): Fixed the read-only flash (flagged iters 7/10/11). `EditorPage` rendered once the *timeline* loaded — often before Firebase auth resolved — so owners briefly saw "read-only / fork". Now gates the editor render on `loading || authLoading` (added `loading` from `useAuth`). Verified: sampling every 150ms across a fresh owner load shows the read-only banner **never appears**, editor is in correct owner/edit mode; signed-out visitors still resolve to read-only. Smoke clean. *Next: dark-mode parity (G4.6) / browse-card hooks (G4.7) / Stream View as first-class view (G3.1).*
- **Iter 12** (a11y + G1.4): Lighthouse audit on the landing = **A11y 98 / SEO 100** (token/ARIA work paid off). Fixed the 2 concrete a11y wins: logo button `aria-label` now includes its visible text "PowerTimeline" (label-content-name-mismatch), and the logo wordmark `h6`→`component="span"` (removes a stray heading). Also fixed **G1.4 "Invalid Date"** — added `formatDateSafe()` to `lib/time.ts` and applied it to HomePage (×2) + UserProfilePage (×2); invalid/missing dates now render "—" (verified on dev). *(Full heading-order fix remains G2.5.)* Smoke clean. *Next: auth-race read-only flash, or Stream View as first-class view (G3.1).*
- **Iter 11** (mobile/responsive — G1.3): At **true 390px** (device emulation), the R2 "critical" mobile Stream View symptoms (time-overprints-title, dual scrollbars, half-empty modal, clipped header) **did NOT reproduce** — they were artifacts of the clamped 500px *non-mobile* test. The body is clean/readable (time renders at the card bottom, not over the title). Fixed the one real issue: the timeline **minimap (z-1400) overlapped the Stream View header** on mobile → added `max-md:hidden` so it hides on small screens while the stream modal is open; header now shows full "Stream View". Mobile landing verified: **no horizontal overflow**. Smoke clean. *Next: a11y/contrast pass or another item.*
- **Iter 10** (UX — G6.5): Added an **editor empty-state** for owners of a 0-event timeline (the onboarding gap found in iter 7) — a centered Calm Modern overlay (indigo icon badge, "Your timeline is empty", indigo "Add your first event" button wired to `openCreate`, "or import a timeline" link). Presentational overlay only (pointer-events pass through; engine untouched); gated `!isReadOnly && events.length === 0`. Verified: shows on an empty timeline, button opens the authoring dialog, hidden once events exist. Smoke clean. *Next: mobile-responsive pass (390px) / verify iter-2 timestamp-overlap.*
- **Iter 9** (content/credibility — G5.2): Replaced the stale version-numbered "Product Roadmap" (which marked v0.5.37 "current" and listed shipped AI/sharing/versioning as *future*) with a credible **"Everything you need, already here"** — 8 accurate shipped-capability cards (timeline builder, AI assistant, source-linked events, Stream view, sharing & embeds, import/export, live current events, automation API) + a small **"Coming next"** trio. No semver in marketing. Removed 3 now-unused icon imports. Smoke clean. *Next: editor empty-state CTA (G6.5) or mobile pass.*
- **Iter 8** (visual): Gave the landing **persona cards distinct indigo icon badges** (newspaper/history_edu/school/public/podcasts/insights) — addresses Round 1's "most generic-AI element" finding (G4.3); refined hover. Refined the **avatar palette** (`avatarUtils.getUserColor`) to a cohesive Calm Modern cool/indigo-led set (dropped loud amber/red/orange/lime) — devtest1 avatar now indigo `#5b5bd6` not blue (G4.2). Smoke clean. *Next: editor empty-state CTA (G6.5) or content cycle (G5.2 roadmap).*
- **Iter 7** (editor audit — found+fixed a bug, VISUAL polish; engine math untouched): Reached the editor as owner `devtest1`. **Found & FIXED a real authoring bug** — creating an event with the *optional* Time left blank failed (`setDoc` rejects `time: undefined`); `addEvent` now strips undefined fields, verified an event saves with no time. Recolored the off-brand **selected event card + anchor** from amber/blue → Calm Modern indigo (`DeterministicLayoutComponent.tsx` highlight styles + `Node.tsx` ring). Authoring dialog already reads clean Calm Modern. **New findings logged:** (a) empty editor has NO "add first event" guidance (onboarding gap, G6.5); (b) editor showed read-only briefly after reload even as owner (possible auth-race — verify). Smoke clean. *Next: persona-card icons (G4.3), blue avatar→indigo, or the editor empty-state CTA.*
